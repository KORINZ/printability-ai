...
data_path = f"output/ALG_HA_old_GG_at_{FREQUENCY}hz.csv"
...
Y = data[["G'", "G''", "tan_delta", "eta_star"]].values
...
for i, name in enumerate(["G'", "G''", "tan_delta", "eta_star"]):
    y = Y[:, i].reshape(-1, 1)
...
def plot_3d_surface(model, title, scaler_X, scaler_Y, data, index, save_fig=False):
    ...
    if title == "G'":
        ...
        zlabel = r"$G'$ [Pa]"
        marker = "<"
        ax.set_title(f"Storage Modulus" + r" ($G'$)")
    elif title == "G''":
        cmap = "inferno"
        zlabel = r"$G''$ [Pa]"
        marker = ">"
        ax.set_title(f"Loss Modulus" + r" ($G''$)")
        ...
        zlabel = r"$|\eta*|$ [Pa·s]"
        marker = "X"
        ax.set_title(f"Magnitude of Complex Viscosity" + r" ($|\eta*|$)")
...
def plot_actual_vs_predicted_cv(
    ...
    if title == "G'":
        ...
        unit = "-"
        marker = "D"
        plt.title(INK_NAME + r" ($\tan(\delta)$)")
    elif title == "eta_star":
        unit = "Pa·s"
        marker = "X"
        plt.title(INK_NAME + r" ($|\eta*|$)")
