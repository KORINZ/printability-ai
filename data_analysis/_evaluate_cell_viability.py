...
parameters = {
    ...
    "MORPH_ITERATIONS": 1,  # Increase to remove more noise
    "MIN_CIRCULARITY": 0.40,
    "GREEN_THRESHOLD": 10,  # Increase to count more yellow cells as dead cells
    "COLOR_INTENSITY_THRESHOLD": None,  # Increase to remove more low intensity regions
}
...
def get_json_path(image_path: str) -> None | str:
    ...
    json_path = "/".join(
        image_path.split("/")[:-2] + [image_path.split("/")[-3] + "_parameters.json"]
    )
...
def update_parameters_based_on_image_path(
    image_path: str, print_output: bool = False
) -> None:
    """Update the parameters based on the image path."""
    json_path = get_json_path(image_path)
    if json_path is not None:
        json_parameters = get_json_parameters(json_path)
        if print_output:
            print(f"Parameters updated based on JSON file: {json_path}", "\n")
            print(json_parameters, "\n")
        return
    elif "10X" in image_path:
        parameters.update(
            {
                ...
                "COLOR_INTENSITY_THRESHOLD": 140,
            }
        )
    elif "4X" in image_path:
        parameters.update(
            {
                "MAXIMUM_CELL_AREA": 800,
